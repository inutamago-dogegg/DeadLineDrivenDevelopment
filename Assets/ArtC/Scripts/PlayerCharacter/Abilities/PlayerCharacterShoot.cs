...
namespace ArtC.PlayerCharacter.Abilities {
    public class PlayerCharacterShoot : PlayerCharacterAbilityBase {
#region Fields

        [SerializeField] private Projectile.Projectile _projectilePrefab;

        [Header("パラメーター")] [SerializeField] private ShootParamEnhanceCurves _basicCurves;
...
#region Unity Callbacks
        ...
        private void Update() {
            _shootDirection = ComputeShootDirection();
        }
...
#region Functions
        ...
        private void ApplyBless(BlessTypes blessType) {
            switch (blessType) {
        ...
        private Vector2 ComputeShootDirection() {
            var mousePosition = (Vector2)_mainCamera.ScreenToWorldPoint(Input.mousePosition);
            var playerCharacterPosition = (Vector2)transform.position;

            var shootDirection = (mousePosition - playerCharacterPosition).normalized;

            if (shootDirection.DEStrictlyEquals(Vector2.zero)) {
